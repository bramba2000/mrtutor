...
**Status: resolved (session expiration hardening).** `Service.Authenticate` now checks session age on every request: rejected once `created_at` is older than `auth.SessionMaxAge` (7d) or `last_seen_at` is older than `auth.SessionIdleTimeout` (3h), both `const` in `auth/service.go`. A stolen token is no longer valid indefinitely between GC runs — the per-request check is the enforcement; `CleanupExpiredSessions` is now a backstop that reclaims rows using the same two constants, rather than the policy itself. `SessionStore.Revoke` was replaced by `Delete` (hard delete on logout); `revoked_at` was dropped from the schema (migration `0003`) in favor of a new `last_seen_at` column. `expires_at` was deliberately not added — expiry is still a Go-side computation from `created_at`/`last_seen_at`, not a stored column, so changing either window is a constant edit rather than a migration.
...
**Status: resolved (Phase 6), plus follow-on fixes.** `auth.Stores` now has `Session`; `sqlite/uow.go`'s `BuildUow` (a new generic, not the old `DB.InTx`) drives a transaction and rebuilds `authsqlite` stores from the same `*sql.Tx` for both read and write — closing the read-your-own-write trap by construction, exactly as recommended. `Service.Register` wraps both `Create` calls in one `RunInTx`, hashing the password and minting the token beforehand as specified. Two things flagged during reconciliation have since been fixed: (1) the old `sqlite.DB.InTx` this section describes, left dead by Phase 6 building a separate mechanism, has been deleted; (2) `uow.go`'s `RunInTx` no longer discards the rollback error — it now returns `errors.Join(err, tx.Rollback())`. The regression test this fix is supposed to be provable by now exists too: `auth/authsqlite/build_test.go`'s `TestUnitOfWork_RunInTx_RollsBackOnFailure` forces the session insert to fail via an FK violation and asserts the principal does not survive, exercising the real `sqlite`/`authsqlite` stack rather than mocks.
...
- **`cmd/api/routes.go` had two route-registration functions**, one dead. **Fixed:** the unused exported `RegisterRoutes` was deleted; `registerRoutes` (unexported, taking `cfg config.Config`) is now the only one.
- **`sqlite.DB.InTx` was dead code again.** §4.7 flagged it as zero-caller before Phase 6; Phase 6 solved the same problem with a new, separate mechanism (`sqlite/uow.go`) rather than using it. **Fixed:** deleted rather than left dead.
- **`sqlite/uow.go`'s `RunInTx` dropped the rollback error.** Unlike `TranslateSQLError`'s accepted-as-is `%v` fallback (#14) or the now-deleted `db.go` `InTx`'s rollback formatting (#15, accepted as-is only because that method was dead) — this was a *fresh* instance of the same defect class, in the transaction path `Register` actually depends on. **Fixed:** now returns `errors.Join(err, tx.Rollback())`.
- **The Phase 1/3 claim that readiness moved from `/healthz` to `/readyz` is false.** `cmd/api/run.go` still mounts the readiness handler at `/healthz`. See §4.7's updated status line. **Still open** — not part of this fix pass (deliberately deferred; the user asked to skip the `expires_at` migration and this route rename wasn't bundled with it either).
- **Defect #1's fix had no regression test.** `Login`'s enumeration fix (not-found → `ErrInvalidCredentials`, dummy-hash comparison) is implemented, but no unit or integration test compared the unknown-username and wrong-password responses. **Fixed:** `auth/service_test.go` and `test/integration/auth_test.go` both gained a subtest asserting the two failures are indistinguishable (same error value; byte-identical HTTP status and body).
- **Phase 6's own acceptance test didn't exist.** The transactional-`Register` mechanism is implemented and live, but nothing forced the session insert to fail and asserted the principal row doesn't survive. **Fixed:** `auth/authsqlite/build_test.go`'s `TestUnitOfWork_RunInTx_RollsBackOnFailure`.
- **`httpx/accesslog.go`'s `recorder` doesn't implement `Unwrap() http.ResponseWriter`.** Phase 4's own plan (§8) calls this out as a requirement for any status-capturing `ResponseWriter` wrapper; the one that got built doesn't meet it. Latent — no current handler needs `Flusher`/`Hijacker` — but a landmine for the next one that does. **Still open** — not part of this fix pass.
- **`Router.Group` has no real consumer.** Phase 7's session middleware was wired by wrapping one handler directly (`requireSession(httpx.WrapNoInput(...))` around `GET /auth/me`) rather than via `r.Group(...)`, so the `slices.Clone`-on-`Group` design Phase 4 built specifically to avoid a sibling-group middleware leak has never been exercised by production code — only by its own test. **Still open** — not part of this fix pass.
...
**Status: done, including the proof test.** There is no standalone `sqlite.Conn` type — `auth/authsqlite.Build` passes the same `*sql.Tx` as both read and write handles inside `sqlite.BuildUow`'s callback, which gets the same correctness property without the named type. `auth.Stores` gained `Session` (not `Sessions`); password hashing and token minting happen before the transaction opens; `Login` bypasses the UoW, all exactly as specified. `auth/authsqlite/build_test.go`'s `TestUnitOfWork_RunInTx_RollsBackOnFailure` is the regression test — it forces the session insert to fail via an FK violation and asserts the principal doesn't survive. See `docs/architecture-tasks.md` Phase 6.
...
**Status: gap closed, different shape (session expiration hardening).** Migration `0003` landed, but adds `last_seen_at` and drops `revoked_at` rather than adding `expires_at` — expiry is still computed, not stored, deliberately, so the two windows (`auth.SessionMaxAge` = 7d, `auth.SessionIdleTimeout` = 3h) stay constant edits rather than migrations. The token-oracle gap is closed by deletion rather than unification: `ErrSessionRevoked` was removed, `Revoke` became `Delete` (hard delete), and missing/idle-expired/cap-expired/logged-out sessions all now yield the single existing `ErrUnauthenticated` — no new `ErrInvalidSession` sentinel was needed. See `docs/architecture-tasks.md`, Phase 7's `expires_at` item, for the full rationale.
...
- **No enumeration:** login with an unknown user and with a wrong password return byte-identical status and body. — **Passes**, now proven by a test in both `auth/service_test.go` and `test/integration/auth_test.go`.
- **`Register` is atomic:** with the session insert forced to fail, no `users` row survives. — **Passes**, proven by `auth/authsqlite/build_test.go`'s `TestUnitOfWork_RunInTx_RollsBackOnFailure`.
